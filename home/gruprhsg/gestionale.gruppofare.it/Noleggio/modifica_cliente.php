...
// Dati cliente
$stmt = $conn->prepare("SELECT nome_cliente, email, telefono, indirizzo, immagine FROM clienti_noleggio WHERE id=? AND azienda_id=?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nome !== '') {
        $stmt = $conn->prepare("UPDATE clienti_noleggio SET nome_cliente=?, email=?, telefono=?, indirizzo=?, immagine=? WHERE id=? AND azienda_id=?");
