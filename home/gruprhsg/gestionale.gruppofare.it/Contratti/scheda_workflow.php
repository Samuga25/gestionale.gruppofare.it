...
// Recupera TUTTI i documenti e li separa in PHP (più robusto di fare il filtro in SQL)
$stmt = $conn->prepare("SELECT * FROM clienti_contratti_documenti WHERE cliente_contratto_id=? AND (tipo_documento IS NULL OR tipo_documento NOT IN ('conferma_ordine', 'conferma ordine', 'Conferma Ordine')) ORDER BY data_upload DESC");
$stmt->bind_param('i', $contratto_id);
$stmt->execute();
$result = $stmt->get_result();
$documenti = [];
...
// Query separata per conferma ordine
$stmt2 = $conn->prepare("SELECT * FROM clienti_contratti_documenti WHERE cliente_contratto_id=? AND tipo_documento IN ('conferma_ordine', 'conferma ordine', 'Conferma Ordine') ORDER BY data_upload DESC");
...
<div class="info-box">
        ...
        <?php elseif ($step_corrente == 5): ?>
            ...
            <div class="info-box">
                ...
                <?php else: ?>
                    ...
                    <div class="alert alert-info">
                        <i class="fas fa-eye"></i> Monitoraggio Step 5. Le azioni sono gestite da Tecnico e Agente.
                    </div>
                    <?php if ($pdf_verbale_firmato): ?>
                    <div class="alert alert-success">
                        <i class="fas fa-check-circle"></i> Verbale firmato caricato.
                        <?php if (!empty($contratto['data_conferma_attivazione'])): ?>
    ...
    <!-- ===== COLONNA DOCUMENTI (destra) ===== -->
    <div class="documents-sidebar">
        <!-- DOCUMENTI -->
        <div class="info-box" id="sezione-documenti">
            <h6><i class="fas fa-folder-open"></i> Documenti (<?= count($documenti) ?>)</h6>
            <?php if (count($documenti) > 0): ?>
                <?php foreach ($documenti as $doc): ?>
                <?php
                    $doc_tipo = strtolower(trim($doc['tipo_documento'] ?? ''));
                    if ($ruolo_utente === 'installatore' && in_array($doc_tipo, ['contratto', 'allegato_a'])) continue;
                    if ($doc_tipo === 'conferma_ordine' || $doc_tipo === 'conferma ordine') continue;
        ...
        <!-- ALLEGATI ALTRO (solo admin, backoffice) -->
        <?php if (in_array($ruolo_utente, ['admin', 'backoffice'])): ?>
