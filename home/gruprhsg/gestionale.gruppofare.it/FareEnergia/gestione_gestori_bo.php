...
if ($ruolo_utente !== 'admin') {
    die('
        <div style="display: flex; justify-content: center; align-items: center; height: 100vh; background: linear-gradient(135deg, #667eea, #764ba2); font-family: Arial, sans-serif;">
            <div style="background: white; padding: 40px; border-radius: 15px; text-align: center; box-shadow: 0 10px 40px rgba(0,0,0,0.3);">
                ...
                <h2 style="color: #dc3545; margin-bottom: 10px;">Accesso Negato</h2>
                <p style="color: #666; margin-bottom: 30px;">Solo gli amministratori possono accedere a questa pagina.</p>
                <a href="contratti_luce_gas.php" style="background: linear-gradient(135deg, #667eea, #764ba2); color: white; padding: 12px 30px; border-radius: 10px; text-decoration: none; display: inline-block;">
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add') {
    ...
    if (empty($nome_gestore)) {
        ...
        try {
            ...
            $stmt = $conn->prepare("SELECT id FROM gestori_bo WHERE nome = ?");
            $stmt->bind_param('s', $nome_gestore);
            $stmt->execute();
            $result = $stmt->get_result();

            if ($result->num_rows > 0) {
                $error = 'Questo gestore BO esiste già!';
            } else {
                // Inserisci nuovo gestore
                $stmt = $conn->prepare("INSERT INTO gestori_bo (nome, attivo, creato_da, data_creazione) VALUES (?, 1, ?, NOW())");
                $stmt->bind_param('si', $nome_gestore, $user_id);
                $stmt->execute();
                $message = 'Gestore BO aggiunto con successo!';
            }
            $stmt->close();
        } catch (Exception $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'edit') {
    ...
    if (empty($nome_gestore)) {
        ...
        try {
            $stmt = $conn->prepare("UPDATE gestori_bo SET nome = ?, modificato_da = ?, data_modifica = NOW() WHERE id = ?");
            $stmt->bind_param('sii', $nome_gestore, $user_id, $gestore_id);
            $stmt->execute();
            $stmt->close();
            $message = 'Gestore BO modificato con successo!';
        } catch (Exception $e) {
            $error = 'Errore: ' . $e->getMessage();
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete') {
    ...
    try {
        $stmt = $conn->prepare("DELETE FROM gestori_bo WHERE id = ?");
        $stmt->bind_param('i', $gestore_id);
        $stmt->execute();
        $stmt->close();
        $message = 'Gestore BO eliminato con successo!';
    } catch (Exception $e) {
        $error = 'Errore: ' . $e->getMessage();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'toggle') {
    ...
    try {
        $stmt = $conn->prepare("UPDATE gestori_bo SET attivo = ? WHERE id = ?");
        $stmt->bind_param('ii', $attivo, $gestore_id);
        $stmt->execute();
        $stmt->close();
        $message = 'Stato gestore BO aggiornato!';
    } catch (Exception $e) {
        $error = 'Errore: ' . $e->getMessage();
    }
}
...
$gestori = [];
try {
    $stmt = $conn->query("SELECT g.*, u.nome as creato_da_nome FROM gestori_bo g LEFT JOIN utenti u ON g.creato_da = u.id ORDER BY g.nome ASC");
    while ($row = $stmt->fetch_assoc()) {
        $gestori[] = $row;
    }
} catch (Exception $e) {
    $error = 'Errore caricamento gestori BO: ' . $e->getMessage();
}
?>
...
<head>
    ...
    <title>Gestione Gestori BO</title>
...
<div class="container-main">
        ...
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h1 class="header-title">
                    ...
                    Gestione Gestori BO
                    <span class="badge-bo">BACKOFFICE</span>
                </h1>
                <p class="text-muted mb-0">
                    Gestisci la lista dei gestori per il campo "Gestore BO" (visibile solo ad Admin e Backoffice)
                </p>
            </div>
            <div>
                <a href="gestione_gestori.php" class="btn btn-outline-primary me-2">
                    <i class="fas fa-building me-2"></i>
                    Gestori Standard
                </a>
    ...
    <div class="card-gestori">
        ...
        <h5 class="mb-3">
            ...
            Lista Gestori BO (<?= count($gestori) ?>)
        </h5>
                ...
                <tbody>
                    ...
                    <tr>
                        <td colspan="6" class="text-center text-muted py-4">
                            ...
                            Nessun gestore BO presente. Aggiungine uno!
                        </td>
                    </tr>
                    <?php else: ?>
        ...
        <div class="modal-content">
            ...
            <form method="POST">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label">Nome Gestore BO</label>
...
$('.btn-delete').on('click', function() {
    if (confirm('Sei sicuro di voler eliminare questo gestore BO?\n\nATTENZIONE: Questa azione è irreversibile!')) {
...
$('.toggle-form').on('submit', function(e) {
    ...
    if (!confirm('Sei sicuro di voler ' + azione + ' questo gestore BO?')) {
        e.preventDefault();
        return false;
    }
});
