...
echo '<pre style="background:#ff0;padding:10px;position:fixed;top:0;left:0;z-index:9999;max-height:300px;overflow:auto;">';
echo "TUTTI I DOCUMENTI:\n";
$stmt_debug = $conn->prepare("SELECT id, tipo_documento, nome_file FROM clienti_contratti_documenti WHERE cliente_contratto_id=?");
$stmt_debug->bind_param('i', $contratto_id);
$stmt_debug->execute();
$r = $stmt_debug->get_result();
while ($row = $r->fetch_assoc()) {
    echo "ID:{$row['id']} | tipo:[{$row['tipo_documento']}] | file:{$row['nome_file']}\n";
}
$stmt_debug->close();
echo '</pre>';
    // Recupera lista PDF
