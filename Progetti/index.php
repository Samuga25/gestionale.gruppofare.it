...
if ($ruolo === 'admin' || $ruolo === 'backoffice') {
    ...
    // Mostra progetti creati dal capoarea/suoi agenti OPPURE con card assegnate a loro
    $stmt_proj = $conn->prepare("SELECT DISTINCT p.*, u.nome as creato_da_nome 
                       FROM progetti p 
                       LEFT JOIN utenti u ON p.created_by = u.id 
                       LEFT JOIN pipeline_boards pb ON pb.progetto_id = p.id
                       LEFT JOIN pipeline_cards pc ON pc.board_id = pb.id
                       WHERE p.attivo = 1 
                       AND (
                           p.created_by IN ($placeholders)
                           OR pc.assegnato_a IN ($placeholders)
                       )
                       ORDER BY p.data_creazione DESC");
    
    // I placeholder sono duplicati (una volta per created_by, una per assegnato_a)
    $allIds = array_merge(array_values($agentIds), array_values($agentIds));
    $stmt_proj->bind_param(str_repeat('i', count($allIds)), ...$allIds);
    $stmt_proj->execute();
    $progetti = $stmt_proj->get_result()->fetch_all(MYSQLI_ASSOC);
    $stmt_proj->close();
} elseif ($ruolo === 'agente') {
    // Mostra progetti creati dall'agente OPPURE con card assegnate a lui
    $stmt_proj = $conn->prepare("SELECT DISTINCT p.*, u.nome as creato_da_nome 
                       FROM progetti p 
                       LEFT JOIN utenti u ON p.created_by = u.id 
                       LEFT JOIN pipeline_boards pb ON pb.progetto_id = p.id
                       ...
                       ORDER BY p.data_creazione DESC");
    $stmt_proj->bind_param("ii", $user_id, $user_id);
    $stmt_proj->execute();
    $progetti = $stmt_proj->get_result()->fetch_all(MYSQLI_ASSOC);
    $stmt_proj->close();
}
