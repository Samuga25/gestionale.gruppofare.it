...
<head>
    ...
    <style>
        ...
        .sponsor-track {
            ...
            width: max-content;
            padding: 20px 0;
            animation: sponsorScroll 220s linear infinite;
        }
        .sponsor-track:hover,
        ...
        .categoria-btn.cat-utilitaria { color: #198754; border-color: #198754; }
        .categoria-btn.cat-utilitaria.active { background: #198754; border-color: #198754; color: white; }
        .categoria-btn.cat-utilitaria:hover:not(.active) { background: rgba(25,135,84,0.1); }
        .categoria-btn.cat-suv { color: #0d6efd; border-color: #0d6efd; }
        .categoria-btn.cat-suv.active { background: #0d6efd; border-color: #0d6efd; color: white; }
        .categoria-btn.cat-suv:hover:not(.active) { background: rgba(13,110,253,0.1); }
        .categoria-btn.cat-crossover { color: #fd7e14; border-color: #fd7e14; }
        .categoria-btn.cat-crossover.active { background: #fd7e14; border-color: #fd7e14; color: white; }
        .categoria-btn.cat-crossover:hover:not(.active) { background: rgba(253,126,20,0.1); }
        .categoria-btn.cat-premium { color: #6f42c1; border-color: #6f42c1; }
        .categoria-btn.cat-premium.active { background: #6f42c1; border-color: #6f42c1; color: white; }
        .categoria-btn.cat-premium:hover:not(.active) { background: rgba(111,66,193,0.1); }
        .categoria-btn.cat-veicoli { color: #20c997; border-color: #20c997; }
        .categoria-btn.cat-veicoli.active { background: #20c997; border-color: #20c997; color: white; }
        .categoria-btn.cat-veicoli:hover:not(.active) { background: rgba(32,201,151,0.1); }
        ...
        @media (max-width: 768px) {
            ...
            .sponsor-header-actions { flex-wrap: nowrap; overflow-x: auto; padding-bottom: 10px; }
            .categoria-btn { flex-shrink: 0; }
        }
        .sponsor-arrow {
            position: absolute;
            top: 50%;
...
<div class="d-flex gap-3 flex-wrap justify-content-center">
                    <button type="button" class="categoria-btn active" data-categoria="" onclick="filterSponsors(this)">
                        <i class="fas fa-border-all me-1"></i>Tutte
                    </button>
                    <button type="button" class="categoria-btn cat-utilitaria" data-categoria="Utilitaria" onclick="filterSponsors(this)">
                        <i class="fas fa-car me-1"></i>Utilitaria
                    </button>
                    <button type="button" class="categoria-btn cat-suv" data-categoria="Suv" onclick="filterSponsors(this)">
                        <i class="fas fa-truck-monster me-1"></i>Suv
                    </button>
                    <button type="button" class="categoria-btn cat-crossover" data-categoria="Crossover" onclick="filterSponsors(this)">
                        <i class="fas fa-route me-1"></i>Crossover
                    </button>
                    <button type="button" class="categoria-btn cat-premium" data-categoria="Premium" onclick="filterSponsors(this)">
                        <i class="fas fa-crown me-1"></i>Premium
                    </button>
                    <button type="button" class="categoria-btn cat-veicoli" data-categoria="Veicoli_commerciali" onclick="filterSponsors(this)">
                        <i class="fas fa-shuttle-van me-1"></i>Veicoli Commerciali
                    </button>
                </div>
        ...
        <div class="sponsor-track-wrapper" id="sponsorWrapper" style="position:relative;">
            <button class="sponsor-arrow sponsor-arrow-left" onclick="scrollSponsors(-1)" id="arrowLeft"><i class="fas fa-chevron-left"></i></button>
            <button class="sponsor-arrow sponsor-arrow-right" onclick="scrollSponsors(1)" id="arrowRight"><i class="fas fa-chevron-right"></i></button>
...
if (!empty($sponsors)):
    ...
    for ($loop = 0; $loop < $loop_count; $loop++):
        foreach ($sponsors as $sp): ?>
                        <div class="sponsor-slide" data-categoria="<?= htmlspecialchars($sp['categoria'] ?? 'Utilitaria') ?>">
...
<div class="content-container">
    <div class="menu-card">
        ...
        <div class="menu-grid">
            ...
            <!-- Segnalazioni -->
            <a href="#" class="menu-item" onclick="openSegnalazioniModal(); return false;" style="background: linear-gradient(135deg, rgba(255,193,7,0.08), rgba(255,193,7,0.15)); border-color: rgba(255,193,7,0.3);">
...
(function() {
    ...
    function scrollSponsors(direction) {
        const track = document.getElementById('sponsorTrack');
        const visibleSlides = Array.from(track.children).filter(s => s.style.display !== 'none');
        ...
        window.scrollResumeTimeout = setTimeout(() => {
            track.style.animation = 'sponsorScroll 220s linear infinite';
...
function filterSponsors(btn) {
    const selected = btn.dataset.categoria;
    document.querySelectorAll('.categoria-btn').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    const slides = document.querySelectorAll('.sponsor-slide');
    let visibleCount = 0;
    slides.forEach(slide => {
        if (!selected || slide.dataset.categoria === selected) {
    ...
    const wrapper = document.getElementById('sponsorWrapper');
