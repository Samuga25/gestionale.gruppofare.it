<?php
// Impedisce che warning/notice rompano il JSON di risposta
ini_set('display_errors', 0);
error_reporting(E_ALL);
ob_start(); // cattura qualsiasi output spurio prima degli header
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'sdoppia_dual') {
    ...
    try {
        // Carica contratto principale — accetta DUAL, LUCE e GAS (multi-fornitura gestita dalle forniture aggiuntive)
        $stmt = $conn->prepare("SELECT * FROM contratti_luce_gas WHERE id = ? LIMIT 1");
        $stmt->bind_param('i', $contratto_id);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        $tipo_energia = strtolower(trim($c['tipo_contratto_energia'] ?? ''));

        // Verifica che il contratto sia sdoppiabile:
        // - DUAL: ha sia POD che PDR (sdoppiamento classico luce + gas)
        // - LUCE con forniture aggiuntive (multi-POD)
        // - GAS con forniture aggiuntive (multi-PDR)
        // Il controllo sulle forniture aggiuntive avviene più avanti;
        // per LUCE/GAS senza forniture aggiuntive blocchiamo subito.
        $is_dual = $tipo_energia === 'dual'
                || (!empty($c['pod']) && !empty($c['pdr']));

        if (!$is_dual && !in_array($tipo_energia, ['luce', 'gas'])) {
            http_response_code(400);
            echo json_encode(['success' => false, 'message' => 'Tipo contratto non sdoppiabile']);
            exit;
        }
        ...
        // -------------------------------------------------------
        // CASO A: nessuna fornitura aggiuntiva
        // -------------------------------------------------------
        $nuovi_ids = [];

        if (empty($forniture)) {
            // LUCE o GAS puri senza forniture aggiuntive: non c'è nulla da sdoppiare
            if (!$is_dual) {
                http_response_code(400);
                echo json_encode(['success' => false, 'message' => 'Nessuna fornitura aggiuntiva trovata: il contratto non può essere sdoppiato']);
                exit;
            }

            // DUAL classico → split in Luce + Gas
        ...
        // - DUAL: ogni fornitura con POD → LUCE, ogni fornitura con PDR → GAS
        // - LUCE puro (multi-POD): tutte le forniture → LUCE
        // - GAS puro (multi-PDR): tutte le forniture → GAS
        // Il contratto originale viene aggiornato con la prima fornitura.
        // -------------------------------------------------------

        // Costruisci lista completa: POD/PDR del contratto principale + forniture aggiuntive
        $tutte_forniture = [];
        ...
        // Determina il tipo corretto per ogni fornitura rispettando il tipo del contratto padre:
        // - LUCE puro → tutte le forniture sono LUCE (i POD)
        // - GAS puro  → tutte le forniture sono GAS  (i PDR)
        // - DUAL      → si basa su pod/pdr presenti nella singola fornitura
        $getTipoFornitura = function(array $f) use ($tipo_energia): string {
            if ($tipo_energia === 'luce') return 'luce';
            if ($tipo_energia === 'gas')  return 'gas';
            // dual
            $ha_pod = !empty($f['pod']);
            $ha_pdr = !empty($f['pdr']);
            if ($ha_pod && $ha_pdr) return 'dual';
            if ($ha_pod) return 'luce';
            return 'gas';
        };

        // Prima fornitura: aggiorna il contratto originale
        $prima       = $tutte_forniture[0];
        $tipo_primo  = $getTipoFornitura($prima);
        ...
        $stmt_upd->bind_param('ssssddiii',
            $tipo_primo, $tipologia_upd,
            $pod_upd, $pdr_upd, $societa_upd,
            $pot_prima, $con_prima,
            $user_id, $contratto_id
        );
        if (!$stmt_upd->execute()) throw new Exception('Errore aggiornamento contratto principale: ' . $stmt_upd->error);
        $stmt_upd->close();
        registraLog($conn, $contratto_id, $user_id, 'sdoppiamento', 'tipo_contratto_energia', $tipo_energia, $tipo_primo);
        ...
        for ($i = 1; $i < count($tutte_forniture); $i++) {
            $f      = $tutte_forniture[$i];
            $tipo_f = $getTipoFornitura($f);
            $pod_f  = !empty($f['pod'])            ? $f['pod']            : null;
            $pdr_f  = !empty($f['pdr'])            ? $f['pdr']            : null;
            $soc_f  = !empty($f['societa_attuale']) ? $f['societa_attuale'] : null;
            $pot_f  = !empty($f['potenza_kw'])     ? (float)$f['potenza_kw']   : null;
            $con_f  = !empty($f['consumo_annuo'])  ? (float)$f['consumo_annuo'] : null;
            $nuovo_id = $creaContratto($tipo_f, $pod_f, $pdr_f, $soc_f, $pot_f, $con_f);
            $nuovi_ids[] = ['id' => $nuovo_id, 'tipo' => $tipo_f];
            $copiaSupporto($nuovo_id);
            registraLog($conn, $nuovo_id, $user_id, 'sdoppiamento', 'tipo_contratto_energia', $tipo_energia, $tipo_f);
            $n = $i + 1;
            ...
            $msg = "Sdoppiamento completato! " . count($tutte_forniture) . " contratti totali:\n" . implode("\n", $riepilogo);
        }

        $conn->commit();
        ob_end_clean(); // scarta qualsiasi output spurio accumulato
        ...
        error_log('=== ERRORE SDOPPIAMENTO ===');
        error_log('Messaggio: ' . $erroreMsg);
        error_log('File: ' . $e->getFile() . ' Riga: ' . $e->getLine());
        error_log('Trace: ' . $traccia);
        error_log('tipo_contratto_energia: ' . ($c['tipo_contratto_energia'] ?? 'NULL'));
        error_log('tipologia: ' . ($c['tipologia'] ?? 'NULL'));
        error_log('num forniture aggiuntive: ' . count($forniture));
        error_log('===========================');
        ob_end_clean(); // scarta output spurio
        http_response_code(500);
