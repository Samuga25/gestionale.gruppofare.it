...
elseif ($contratto_id > 0) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && (isset($_POST['update_contratto']) || isset($_POST['save_contratto']))) {
        ...
        if ($contratto['tipo_settore'] === 'telecomunicazioni' && empty($tipo_contratto_energia)) {
            $tipo_contratto_energia = 'telefonia';
        }
    ...
    <div class="header-contratto">
        <div class="row align-items-center">
            ...
            <div class="col-md-3 text-end">
                ...
                <?php endif; ?>
                <?php
                $can_sdoppia_scheda = false;
                if ($contratto_id > 0 && $contratto !== null) {
                    $tipo_energia_scheda = strtolower($contratto['tipo_contratto_energia'] ?? '');
                    $is_dual_scheda      = $tipo_energia_scheda === 'dual'
                                       || (!empty($contratto['pod']) && !empty($contratto['pdr']));
                    $can_sdoppia_scheda  = in_array($ruolo_utente, ['admin', 'backoffice'])
                                       && ($is_dual_scheda
                                           || (in_array($tipo_energia_scheda, ['luce', 'gas']) && !empty($forniture_aggiuntive)));
                }
                ?>
                <?php if ($can_sdoppia_scheda): ?>
                <button type="button" class="btn btn-warning btn-lg mb-2 w-100" id="btn-sdoppia-scheda" data-id="<?= $contratto_id ?>" data-nome="<?= htmlspecialchars(($contratto['nome'] ?? '') . ' ' . ($contratto['cognome'] ?? '')) ?>" data-tipo="<?= htmlspecialchars(strtolower($contratto['tipo_contratto_energia'] ?? '')) ?>">
        ...
        <div class="card-section">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-3">
                    ...
                    <?php $tipo_settore = $contratto['tipo_settore'] ?? ''; ?>
                    <?php if ($tipo_settore === 'telecomunicazioni'): ?>
                    <select name="tipo_contratto_energia" class="form-select">
                        <option value="telefonia" <?= ($contratto['tipo_contratto_energia'] ?? '') === 'telefonia' ? 'selected' : '' ?>>Telefonia</option>
                    </select>
                    <?php else: ?>
                    <select name="tipo_contratto_energia" class="form-select">
                        <option value="luce" <?= ($contratto['tipo_contratto_energia'] ?? '') === 'luce' ? 'selected' : '' ?>>Luce</option>
                        <option value="gas" <?= ($contratto['tipo_contratto_energia'] ?? '') === 'gas' ? 'selected' : '' ?>>Gas</option>
                        <option value="dual" <?= ($contratto['tipo_contratto_energia'] ?? '') === 'dual' ? 'selected' : '' ?>>Dual</option>
                    </select>
                    <?php endif; ?>
                </div>

                <!-- TIPOLOGIA (per telecomunicazioni) -->
                <?php if ($tipo_settore === 'telecomunicazioni'): ?>
                <div class="col-md-3">
                    <label class="form-label">Tipologia</label>
                    <select name="tipologia" class="form-select">
                        <option value="portabilita" <?= ($contratto['tipologia'] ?? '') === 'portabilita' ? 'selected' : '' ?>>Portabilità</option>
                        <option value="nuova_attivazione" <?= ($contratto['tipologia'] ?? '') === 'nuova_attivazione' ? 'selected' : '' ?>>Nuova Attivazione</option>
                    </select>
                </div>
                <?php endif; ?>
...
// ============================================================
// SDOPPIA CONTRATTO (dalla scheda) — supporta DUAL, multi-POD, multi-PDR
// ============================================================
$(document).on('click', '#btn-sdoppia-scheda', function () {
    const id   = $(this).data('id');
    const nome = $(this).data('nome');
    const tipo = $(this).data('tipo') || 'dual';
    const btn  = $(this);

    let tipoMsg = '';
    if (tipo === 'dual') {
        tipoMsg = 'Il contratto DUAL verrà separato in un contratto LUCE (per ogni POD) e un contratto GAS (per ogni PDR).';
    } else if (tipo === 'luce') {
        tipoMsg = 'I POD aggiuntivi verranno separati: ogni POD diventerà un contratto LUCE indipendente.';
    } else if (tipo === 'gas') {
        tipoMsg = 'I PDR aggiuntivi verranno separati: ogni PDR diventerà un contratto GAS indipendente.';
    } else {
        tipoMsg = 'Le forniture aggiuntive verranno separate in contratti distinti.';
    }

    if (!confirm(
        'Vuoi sdoppiare il contratto di ' + nome + '?\n\n' +
        tipoMsg + '\n\n' +
