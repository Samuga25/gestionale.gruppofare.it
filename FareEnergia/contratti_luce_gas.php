...
if ($can_access) {
    $sql = "SELECT clg.*, u.nome as agente_nome, COUNT(DISTINCT t.id) as num_ticket_aperti,
                   (SELECT COUNT(*) FROM contratti_luce_gas_forniture f WHERE f.contratto_id = clg.id) as num_forniture_aggiuntive
...
$badge_class = $stato_colors[$c['stato']] ?? 'stato-inserito';
                        ...
                        $tipo_energia = strtolower($c['tipo_contratto_energia'] ?? '');
                        $is_dual      = $tipo_energia === 'dual'
                                     || (!empty($c['pod']) && !empty($c['pdr']));
                        $ha_forniture = (int)($c['num_forniture_aggiuntive'] ?? 0) > 0;
                        // Sdoppiabile se: dual, oppure luce/gas con almeno una fornitura aggiuntiva
                        $can_sdoppia  = in_array($ruolo_utente, ['admin', 'backoffice'])
                                     && ($is_dual || ($ha_forniture && in_array($tipo_energia, ['luce', 'gas'])));
                    ?>
                        <tr>
                            <td>
                            ...
                            <td>
                                ...
                                <?php if ($can_sdoppia): ?>
                                    <button
                                        ...
                                        data-tipo="<?= htmlspecialchars(strtolower($c['tipo_contratto_energia'] ?? '')) ?>"
                                        title="Sdoppia: separa le forniture in contratti distinti">
