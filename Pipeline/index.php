...
foreach ($columns as $col) {
    ...
    if ($ruolo === 'agente') {
        // Agente: vede solo le card assegnate a lui OPPURE create da lui
        $stmt = $conn->prepare("
            SELECT c.*,
            ...
            WHERE c.column_id = ?
              AND (c.assegnato_a = ? OR c.created_by = ?)
            ORDER BY c.posizione ASC
        ");
        $stmt->bind_param("iii", $col_id, $user_id, $user_id);

    } elseif ($ruolo === 'capoarea') {
        // Capoarea: vede le card assegnate/create da lui e dai suoi agenti
        $stmt_ag = $conn->prepare("SELECT id FROM utenti WHERE capoarea_id = ?");
        $stmt_ag->bind_param("i", $user_id);
        $stmt_ag->execute();
        $res_ag   = $stmt_ag->get_result();
        $agentIds = [$user_id];
        while ($row_ag = $res_ag->fetch_assoc()) {
            $agentIds[] = $row_ag['id'];
        }
        $stmt_ag->close();
        ...
        $stmt = $conn->prepare("
            ...
            LEFT JOIN utenti u_creatore ON c.created_by = u_creatore.id
            WHERE c.column_id = ?
              AND (c.assegnato_a IN ($placeholders) OR c.created_by IN ($placeholders))
            ORDER BY c.posizione ASC
        ");
        // bind_param: col_id + agentIds (per assegnato_a) + agentIds (per created_by)
        $allIds = array_merge([$col_id], $agentIds, $agentIds);
        $types  = 'i' . str_repeat('i', count($agentIds)) . str_repeat('i', count($agentIds));
        $stmt->bind_param($types, ...$allIds);

    } else {
        // Admin / backoffice: vedono tutto
        $stmt = $conn->prepare("
            SELECT c.*,
                   u_assegnato.nome as assegnato_nome,
                   u_creatore.nome as creatore_nome,
