...
if ($ruolo === 'admin' || $ruolo === 'backoffice') {
    ...
    $stmt_proj->bind_param(str_repeat('i', count($agentIds)), ...array_values($agentIds));
    $stmt_proj->execute();
    ...
    $stmt_proj = $conn->prepare("SELECT p.*, u.nome as creato_da_nome 
                       FROM progetti p 
                       LEFT JOIN utenti u ON p.created_by = u.id 
                       WHERE p.attivo = 1 AND p.created_by = ?
                       ORDER BY p.data_creazione DESC");
...
<script>
    ...
    function editProject(id) {
        ...
        .then(res => res.json())
        .then(data => {
            if (data.success && data[' progeto']) {
                const p = data[' progeto'];
