...
if ($action === 'add_comment') {
    ...
    // Verifica permessi
    $stmt = $conn->prepare("SELECT c.*, b.settore FROM pipeline_cards c LEFT JOIN pipeline_boards b ON c.board_id = b.id WHERE c.id = ?");
    $stmt->bind_param("i", $card_id);
    $stmt->execute();
    ...
    $settore = $card['settore'] ?? '';
    $is_progetto = (strpos($settore, 'proj_') === 0);
    $is_noleggio = ($settore === 'farenoleggio');
    $ruolo = $_SESSION['ruolo'] ?? '';

    if (($is_noleggio || $is_progetto) && $ruolo === 'agente') {
        if ($card['created_by'] != $user_id && $card['assegnato_a'] != $user_id) {
            jsonResponse(['success' => false, 'error' => 'Non hai i permessi']);
        }
    }
