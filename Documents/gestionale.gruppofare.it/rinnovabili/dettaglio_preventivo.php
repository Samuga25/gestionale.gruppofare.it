...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $tipo === 'standard' && !empty($_FILES['pdf_preventivo']['name'])) {
    ...
    if ($file['error'] === UPLOAD_ERR_OK) {
        ...
        if ($mime === 'application/pdf' && $ext === 'pdf') {
            $nome_unico   = uniqid('prev_', true) . '.pdf';
            $percorso_rel = $upload_url . $nome_unico;
            $percorso_ass = $upload_dir . $nome_unico;
            ...
            if (move_uploaded_file($file['tmp_name'], $percorso_ass)) {
                ...
                $stmt_up->bind_param('ss', $percorso_rel, $id);
                if ($stmt_up->execute()) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['stato'])) {
    ...
    if ($tipo === 'bando') {
        ...
        $stati_validi = ['inattesa', 'preventivo_caricato', 'accettato', 'rifiutato'];
        if (in_array($nuovo_stato, $stati_validi)) {
...
$stati_standard = ['inattesa' => 'In attesa', 'preventivo_caricato' => 'Preventivo caricato', 'accettato' => 'Accettato', 'rifiutato' => 'Rifiutato'];
$stati_map      = $tipo === 'bando' ? $stati_bando : $stati_standard;

$stato_attuale = $d['stato'] ?? 'inattesa';
            ...
            <div class="info-row">
                ...
                <span class="val">
                    <div class="agente-pill">
                        <div class="agente-av"><?= strtoupper(substr($d['agente_nome'] ?? '?', 0, 1)) ?></div>
                        <?= htmlspecialchars($d['agente_nome'] ?? '—') ?>
                    </div>
                </span>
            </div>
            <?php if (!empty($d['agente_email'])): ?>
            <div class="info-row">
                <span class="lbl">Email</span>
                <span class="val"><a href="mailto:<?= htmlspecialchars($d['agente_email']) ?>"><?= htmlspecialchars($d['agente_email']) ?></a></span>
            </div>
            <?php endif; ?>
            <div class="info-row">
